% Stable LaTeX document shell and CV header.
% Keep contact details, packages, macros, and layout here.

\documentclass[letterpaper,10pt]{article}

\usepackage{latexsym}
\usepackage[empty]{fullpage}
\usepackage{titlesec}
\usepackage{marvosym}
\usepackage[usenames,dvipsnames]{color}
\usepackage{verbatim}
\usepackage{enumitem}
\usepackage[hidelinks]{hyperref}
\usepackage{fancyhdr}
\usepackage[english]{babel}
\usepackage{tabularx}
\input{glyphtounicode}

% Font options
\usepackage[sfdefault]{roboto}

\pagestyle{fancy}
\fancyhf{}
\fancyfoot{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}
\setlength{\footskip}{4pt}

\addtolength{\oddsidemargin}{-0.5in}
\addtolength{\evensidemargin}{-0.5in}
\addtolength{\textwidth}{1in}
\addtolength{\topmargin}{-.5in}
\addtolength{\textheight}{1.0in}

\urlstyle{same}
\raggedbottom
\raggedright
\setlength{\tabcolsep}{0in}

% Section formatting
\titleformat{\section}{\Large\bfseries\scshape\raggedright}{}{0em}{}[\titlerule]
\titlespacing*{\section}{0pt}{6pt}{6pt}
\setlist[itemize]{topsep=1.5pt, partopsep=0pt, parsep=0pt, itemsep=2pt}

% Ensure PDF is machine readable
\pdfgentounicode=1

% Custom commands
\newcommand{\resumeItem}[1]{
    \item\small{#1}\vspace{-0.5pt}
}
\newcommand{\resumeSubheading}[4]{
\vspace{2pt}\item
  \begin{tabular*}{0.97\textwidth}[t]{l@{\extracolsep{\fill}}r}
    \textbf{#1} & #2 \\
    \textit{{#3}} & {#4} \\
  \end{tabular*}\vspace{-0pt}
}

\renewcommand\labelitemii{$\vcenter{\hbox{\tiny$\bullet$}}$}
\newcommand{\resumeSubHeadingList}{\begin{itemize}[leftmargin=0.15in, label={}]\vspace{-2pt}}
\newcommand{\resumeSubHeadingListEnd}{\end{itemize}}

\begin{document}

\begin{center}
  \textbf{\Huge Kostiantyn Pysanyi}\vspace{4pt} \\
  \small +49 1525 8447880 $|$ \href{mailto:kostiantyn.pysanyi@gmail.com}{kostiantyn.pysanyi@gmail.com} $|$ Leipzig, Germany $|$
  \href{https://www.linkedin.com/in/kostiantyn-pysanyi/}{linkedin.com/in/kostiantyn-pysanyi} $|$ \href{https://github.com/Abermal}{github.com/Abermal}
\end{center}
% Source of truth for factual CV content.

% MLOPS
% \section{Professional Summary}
% End-to-end Machine Learning Engineer with solid MLOps experience and a strong research foundation. Proven track record in deploying production-scale ML systems, improving accuracy, reducing latency, and driving real-time model performance. Adept at collaborating across science, engineering and product teams to build impactful solutions.


\section{Professional Summary}
Machine Learning Engineer with a strong research foundation and hands-on experience delivering end-to-end computer vision systems. Experienced in building data, training, evaluation, and deployment workflows for real-world imaging applications.

\section{Technical Skills}
\resumeSubHeadingList
  \resumeItem{\textbf{ML \& CV}: \textbf{PyTorch/Lightning}, TensorFlow, JAX, Detectron2, MMEngine/MMCV/MMDetection, Albumentations, Scikit-learn, XGBoost, \textbf{OpenCV}, ITK, pyclesperanto, NumPy, Pandas, \textbf{CVAT}, Transformers/TIMM/Huggingface}
  \resumeItem{\textbf{Experimentation \& Tracking}: \textbf{MLflow}, Tensorboard, Optuna, \textbf{DVC}, Matplotlib/seaborn/Plotly}
  \resumeItem{\textbf{Infrastructure \& Deployment}: Linux, \textbf{Docker}, \textbf{Kubernetes}, \textbf{Prefect}, CI/CD (GitLab, Bitbucket), \textbf{AWS}, Triton/TensorRT}
  \resumeItem{\textbf{Backend \& Dev Tools}: Flask, \textbf{REST}, FastAPI, OpenAPI, FHIR, Git, SQL, MQTT}
  \resumeItem{\textbf{Languages}: \textbf{Python} (Professional), C++17, TypeScript (Proficient), Java, MATLAB (Basic)}
\resumeSubHeadingListEnd

\section{Experience}
\resumeSubHeadingList
  \resumeSubheading
      {Machine Learning \& Computer Vision Engineer}{June 2023 -- July 2026}
      {RAYLYTIC GmbH}{Full-time}
      \resumeSubHeadingList
          \resumeItem{\textbullet\ Delivered production-grade \textbf{segmentation, detection, and classification} models for X-ray imaging; improved accuracy (mAP, Dice, ROC) by 20--50\% over baseline and reduced training time 8-fold}
          \resumeItem{\textbullet\ Built an end-to-end \textbf{2D--3D image registration} pipeline, including mesh processing and alignment, for 3D implant pose estimation from X-rays; improved acceptance rate by 45\% and reduced error by 15\%; productionized it with \textbf{GPU-accelerated Docker} and \textbf{Prefect workflow orchestration}}
          \resumeItem{\textbullet\ Implemented an \textbf{MLOps} pipeline with data versioning via \textbf{DVC}, \textbf{MLflow} experiment tracking, and automated evaluation}
          \resumeItem{\textbullet\ Migrated segmentation model serving to a centralized \textbf{Triton} Inference Server--based \textbf{REST API}, cutting latency by 50\%}
          
          \resumeItem{\textbullet\ Developed \textbf{3D instance segmentation and registration} for light-sheet microscopy images in a liver cancer research project}
      \resumeSubHeadingListEnd
  
  \resumeSubheading
      {Freelance Computer Vision Engineer}{Feb 2026 -- Mar 2026}
      {Smoothr.de / Sterlix GmbH}{Contract}
      \resumeSubHeadingList
          \resumeItem{\textbullet\ Developed depth-map--based camera obstruction detection for real-time retail anti-theft systems}
      \resumeSubHeadingListEnd
      
  \resumeSubheading
      {Research Engineer}{January 2022 -- March 2023}
      {Data Science Research Group}{Part-time}
      \resumeSubHeadingList
          \resumeItem{\textbullet\ Developed ML model for bio-signal classification for pain assessment in OP scenarios without patient feedback}
          \resumeItem{\textbullet\ Designed data pipelines and training workflows for a ticket-routing classifier, transforming unstructured text into structured features and streamlining dataset updates and model evaluation for \hyperlink{https://infai.org/projekt-kmi/}{industry partners}.}
      \resumeSubHeadingListEnd

  \vspace{2pt}
  \textit{\textbf{Internships}}
  \resumeSubHeadingList
      \resumeItem{\textbullet\ {ML Thesis Student at RAYLYTIC GmbH} -- (Oct 2022 -- Feb 2023): Developed anatomical landmark localization model}
      \resumeItem{\textbullet\ {ML Intern at Prudsys GmbH} -- (Dec 2021 -- Feb 2022): Automated analytic workflows and data preprocessing pipelines}
      \resumeItem{\textbullet\ {Autonomous Driving Intern at IAV GmbH} -- (Sep 2021 -- Nov 2021): Investigated inverse {reinforcement learning} for self-driving}
      \resumeItem{\textbullet\ {Student Research Assistant at WHZ} -- (Jan 2020 -- Dec 2021): Developed a \href{https://www.youtube.com/watch?v=Xa7rivGy6WQ}{semantic segmentation model for automated tissue-density analysis of 3D lung CT scans}; developed an {autoML} library and predicted patient admission to the ICU during COVID.}
  \resumeSubHeadingListEnd
\resumeSubHeadingListEnd

\section{Education}
\resumeSubHeadingList
  \resumeSubheading
      {B.Sc.\ Data Science with distinction (1.1 / 1.0)}{Oct 2019 -- Apr 2023}
      {University of Applied Sciences Zwickau}{Zwickau, Germany}
      % \textbullet\ \small{Thesis: Anatomical landmark localization for lumbar segmental angle estimation}
\vspace{-2pt}
  \resumeSubheading
      {M.Sc.\ Civil Engineering (wGPA: 90.12/100)}{Sep 2014 -- Jun 2020}
      {Odesa State Academy of Civil Engineering and Architecture}{Odesa, Ukraine}
      % \textbullet\ \small{Thesis: Dynamics and stability of rectangular orthotropic plates}
\resumeSubHeadingListEnd

\section{Publications \& Awards}
\resumeSubHeadingList
  \resumeItem{\textbf{EFORT 2026}: Validation of a 2D-3D Image Registration Method for TKA Implant Pose Estimation. Pysanyi et al.}
  \resumeItem{\textbf{DWG 2023}: Automatic determination of lumbar segmental lordosis based on an AI algorithm. Pysanyi et al.}
  \resumeItem{\textbf{1st Place}: National Strength of Materials Olympiad (Ukraine, 2018)}
\resumeSubHeadingListEnd

\section{Languages}
\resumeSubHeadingList
  \resumeItem{German (C1, TestDaF), English (C2, EF SET Certificate)}
\resumeSubHeadingListEnd
\end{document}
